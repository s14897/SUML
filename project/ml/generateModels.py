...
def prepare_Data(directory, b_size_1, epochs1, activation1, activation2, output_dims1, b_size2, epochs2, activation3, activation4, activation5, activation6, output_dims2, output_dims3, output_dims4):
    #przygotowanie danych i wczytanie zbioru, wczytujemy nasz plik ktory bylby na "serwerze"
    maindirectory = TRAINING_DATA
    datasetmain = pd.read_csv(maindirectory, sep=';')
    print(f'file: {directory}')
    datasettest =  pd.read_csv(directory, sep=';')

    #czyscimy niepotrzebne kolumny
    ...
    #funkcja do generowania auc roc
    def auc_roc(y_true, y_pred):
        
        value, update_op = tf.contrib.metrics.streaming_auc(y_pred, y_true)

        # wszystkie zmienne dla auc roc
    ...
    #ustawienie callbacka wplywajacego na prezentowanie sie wartosci w konsoli
    ...
    def create_first_model(X_train1, y_train1, X_test, y_test, b_size_1, epoch1, activation1, activation2, output_dims1):
        #ustawienie modelu wedlug wyboru uzytkownika (model prostszy), ustawienie jadra, warstw wejsciowych wyjsciowych, funkcji aktywacji
        #oraz trenowanie sieci oraz ustawienie zbioru walidacyjnego
    ...
    def create_third_model(X_train, y_train, X_test, y_test, b_size_1, epoch1, activation1, activation2, activation3, activation4, output_dims1, output_dims2, output_dims3):
        #ustawienie modelu wedlug wyboru uzytkownika (model trudniejszy)
    ...
    def generate_plot_model_one(X_test, y_test, X_train, y_train, classifier1, history, history_test, X_prob):
        #przygotowanie zmiennych do generowania plotow i ustawienia wykresow dla pierwszego modelu
    ...
    def generate_plot_model_two(X_test, y_test, X_train, y_train, classifier2, history, history_test, X_prob):
        #przygotowanie zmiennych do generowania plotow i ustawienia wykresow dla drugiego modelu
    ...
    #tutaj wywolanie wszystkich funkcji potrzebnych do generowania modeli oraz zwrocenie wynikow
